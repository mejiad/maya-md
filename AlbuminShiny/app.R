  ...
  header_str <- paste("MK-means ", CENTERS, " cluster", sep=" ")
    ...
    output$vitD <- renderPlot({
        df_bmi <- df_albumin %>% filter(BMI.Group >= input$bmiInput[1] & 
                                          ...
                                          Vitamin.B12..Serum < input$vitB12.Input &
                                          Albumin...Serum < input$albumin.Input)  
    ...
    output$cluster <- renderPlotly({
      ...
      albumin_kmean <- albumin_kmean %>% filter(Albumin...Serum < input$albumin.Input & 
                                                  Vit.D.assay < input$vitD.Input & 
                                                  Vitamin.B12..Serum < input$vitB12.Input)
      ...
      cluster_plot <- cluster_plot %>% layout(autosize = F, width = 800, height = 800, scene = list(xaxis = list(title="Albumin"), yaxis=list(title="Vitamin B12"), zaxis=list(title="Vitamin D")) )
      cluster_plot
    })
    ...
    output$vitB12 <- renderPlot({
        ...
        figVitB12 <- df_bmi %>% ggplot(aes(x=Albumin...Serum, y = Vitamin.B12..Serum)) + geom_point(aes(col=factor(BMI.Group))) + xlim(0,6) + 
    ...
    output$datatable <- ({
      renderDT(df_bmi, options = list(lenghtChange = FALSE))
    })
